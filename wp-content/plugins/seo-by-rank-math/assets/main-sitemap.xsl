			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>Locations Sitemap - restaurantetorredevega.es</title>
					</xsl:when>
					<xsl:otherwise>
						<title>XML Sitemap - restaurantetorredevega.es</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="http://localhost/restaurante/sitemap_index.xml">&#8592; Índice del mapa del sitio</a>							</p>
